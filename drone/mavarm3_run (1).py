...
def takeoff(alt):
	...
	while i < 15: #Timer 3 detik dengan time.sleep(3)	
		...
		if bottom_distance >= alt:
			...
			rcover(1500, 1500, 1500, 0, 0, 0, 0, 0) 
			time.sleep(3)
...
def update_sensors():
	...
	while True:
		...
		left_distance = read_sensor('12')
		right_distance = read_sensor('16')
	...
	while True:
		...
		if bottom_distance < 0.5:
			...
			if front_distance > 1: # If 1 meter untuk maju, ngko onk if 0.7 ge mandek
				...
				#			rcover(1500 - nilaikanan,1450,1500,0,0,0,0) #rodok ngiri
							print("Drone rodok ngiri")
						elif front_distance < 0.7: #Kemungkinan iki detect gerbang
							print("Depan ada hambatan, landing!")
							#landing() #Read landing function first or change to changemode(9)
							
							#Kondisi lek pengen lewat gawang
							print("Depan ada hambatan, naik!")
				#			rcover(1500,1500,1600,0,0,0,0,0)
							if front_distance > 2:
				#				rcover(1500,1470,1500,0,0,0,0,0)
								print ("drone maju")
								#time.sleep(5)
								#landing()
								#atau
				...
				#			rcover(1500 - nilaikanan,1450,1500,0,0,0,0) #rodok ngiri
							print("Drone rodok ngiri")
						elif front_distance < 0.7: #HArus e iki detect gawang
							print("Depan ada hambatan, landing!")
...
def program3():
	...
	while bottom_distance is not None and front_distance is not None:
		while not vehicle.motors_armed():
			...
			rcover(1500,1500,1450,1500,65535,65535,65535,65535)
			time.sleep(1)
			front_distance = get_distance(10)
			while front_distance >= 1.2:
				rcover(65535,1450,1450,1500,65535,65535,65535,65535) 
				print(f"maju, {front_distance}")
				right_distance = get_distance(12)
				left_distance = get_distance(16)
				while left_distance < 0.7:
					kiriint = 100 - left_distance*10
					rcover(1500+kiriint,65535,65535,65535,65535,65535,65535,65535)
					print("ke ngiri nen")
				while right_distance < 0.7:
					kananint = 100 - right_distance*10
					rcover(1500-kananint,65535,65535,65535,65535,65535,65535,65535)
					print("nganan bgt bro")
			print("keluar while front")
			rcover(1500,1700,1450,1500,0,0,0,0) #rem
			print("stoppppp")
			time.sleep(0.8)
			"""
			rcover(1410,1500,1450,1500,0,0,0,0)
			time.sleep(2)
			"""
			print("landing.......")
			changemode(9) #land
			if bottom_distance < 0.2:
				time.sleep(2)
				arm(0)
				break
			print("is program3 done?")
			break
		print("program3 done")
	else :
		print("init sensor.....")
